...
export async function updateEvent(eventObj) {
    ...
    try {
        ...
        const db = client.db(dbName);
        const collection = db.collection(eventsCol);
        const res = await collection.updateOne({ _id: id }, {
            $set: {
 ...
 *                            }
 */
export async function getAllEvents(orgName) {
    const client = new MongoClient(uri);
    try {
        await client.connect();
        const db = client.db(dbName);
        const collection = db.collection(eventsCol);
        const res = await collection.find({ organization: orgName })
        if (res) {
            return {
                success: true, 
                msg: "Events found.", 
...
eventsConnect.getAllEvents = getAllEvents;
...
export async function eventRsvp(user, event, rsvpStatus) {
    ...
    try {
        ...
        let res
        if (rsvpStatus === "Yes") {
            res = await collection.updateOne({ _id: eventId }, {
                $addToSet: {
                    rsvpYes: userId
                }
            });
...
async function removeRsvpFromEvent(eventId, userId, collection, value) {
    ...
    switch (value) {
        case "Yes":
            removeStatus = await collection.updateOne({_id: eventId}, {
                $pull: {
                    rsvpYes: userId
                }
            });
